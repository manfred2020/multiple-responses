...
def mr(df, *col, count = 1, save = False):
    ...
    mr_set = (df[columns] == count).sum(axis = 0).sort_values(ascending = True)
    # eine Basisauswertung, zwecks der Reihenfolge für alle weiteren
    mrset = mr_set.sum(axis = 0)
    # Summe der gesamt abgegebenen Antworten ermitteln
    ...
    finalset = pd.DataFrame({"Anzahl d Nennungen" : mr_set,
                          ...
                          "Prozent d tatsächl. Nennungen" : ((mr_set/(mrset))*100).round(2)}
                            ).fillna(0) # fillna() wegen Division durch 0
